import { parse as parse_date } from 'date-fns';

export function extractDate(dateString: string): Date {
  // Example date string: "12:00am Sat 31 December, 2022"
  // returns a date object
  // parse using date-fns
  const date = parse_date(dateString, 'h:mma EEE d MMMM, yyyy', new Date());
  // convert to AEST, assuming the date from the bank is in UTC, don't care about daylight savings
  date.setHours(date.getHours() + 10);
  return date;
}

export function toTitleCase(str: string) {
  return str.replace(
    /\w\S*/g,
    (txt) => txt.charAt(0).toUpperCase() + txt.substring(1).toLowerCase()
  );
}
