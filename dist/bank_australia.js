...
exports.BankAustralia = exports.TransactionType = void 0;
...
(function (TransactionType) {
    ...
    TransactionType["ALL_WDLS"] = "ALL WDLS";
    TransactionType["ALL_DEPOSITS"] = "ALL DEPOSITS";
    TransactionType["BPAY"] = "BPAY";
    TransactionType["VISA"] = "VISA";
})(TransactionType || (exports.TransactionType = TransactionType = {}));
class BankAustralia {
    _actual;
    _idMapping;
    static PayeeName = 'Bank Australia';
    /**
     * Manual overrides for category mapping to ensure some categories don't get created in the app
     */
    static CategoryOverrides = {
     ...
     */
    async importTransactionsAsync(path) {
        ...
        const accountGroupedTransactions = transactions.reduce((acc, transaction) => {
        ...
        for (const accountId in accountGroupedTransactions) {
            await this._actual.importTransactionsAsync(accountId, accountGroupedTransactions[accountId]);
        }
    ...
    readCsv(path) {
        ...
        const csv = (0, sync_1.parse)(data, {
            columns: true,
            skip_empty_lines: true
        });
        return csv;
    }
    async csvDataRowToTransactionAsync(row) {
        // Grab out common values
        const date = (0, utils_1.extractDate)(row['Effective date']);
        const debit = api_1.utils.amountToInteger(parseFloat(row['Debit amount']));
        const credit = api_1.utils.amountToInteger(parseFloat(row['Credit amount']));
        ...
        if (!account) {
            throw new Error(`Unable to find account with account number: ${row['Account number']}`);
        }
        let transaction = {
            account,
            date,
     ...
     */
    async parseTransactionTypeAllAsync(row, transaction) {
     ...
     * @throws Error if the transaction information cannot be parsed or is unhandled
     */
    parseTransactionTypeAllWdls(row, transaction) {
     ...
     * @throws Error if the transaction information cannot be parsed or is unhandled
     */
    parseTransactionTypeAllDeposits(row, transaction) {
     ...
     * @throws Error if the transaction information cannot be parsed or is unhandled
     */
    parseTransactionTypeBPay(row, transaction) {
     ...
     * @throws Error if the transaction information cannot be parsed or is unhandled
     */
    async parseTransactionTypeVisaAsync(row, transaction) {
        ...
        if (merchantName &&
            ...
            payee_name = (0, utils_1.toTitleCase)(match[1]).trim();
        }
...
exports.BankAustralia = BankAustralia;
//# sourceMappingURL=bank_australia.js.map